...
| One-million Store seed/publication, tag plans, manifest reuse, database bytes | Clean scratch commit `37b789b` (AUT-533 transfer provenance): 1,000,000 scale rows plus two foundation pages; 4,000,008 slot rows; 1,300,004 tag memberships; healthy schema v6 with zero FK violations; all canonical/tag/selector plans indexed; 69.116 s seed; 594.347 s publication; 262.675 s idempotent republish; five manifests for 1,000,002 pages; 3,205,939,200 final database bytes; 1,310,748,672 publication-allocation bytes; 591,757,312 maximum resident bytes. | The full-template baseline completed within local resources. Keep the compiler batched, carry progress and idempotency metadata into TiDB, preserve the covering membership and canonical indexes, and test Region/hot-key behavior before choosing partitions. |

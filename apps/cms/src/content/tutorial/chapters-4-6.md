...
| Shape | Variation pressure | Headline evidence | What it does **not** prove |
| --- | --- | --- | --- |
| Dense Eligible Vehicles | Many intersecting scalar dimensions and exact overrides | 24 pages, 24 manifests, 168/168 placements, conflict at `legal-notice` priority `60` | That manifests reduce storage for every template |
| Sparse Stores | Large cardinality, explicit many-to-many tags, few sparse operations | 1,000,002 pages, five manifests, 999,997 reused pages, zero selectors on serve | A TiDB latency SLO or an end-to-end physical byte saving |
| Structural replacement | Schema/renderer change and tombstone at stable placements | `hero` → `hero_alt`, 22/24 unchanged pointers, 91.67% inheritance, exact-hash rollback | A complete production block-schema governance policy |
...
> **Estimated time:** Read 5 min · Media 1 min · Digest 3 min
> **Learning outcome:** Publish the current persisted revisions, inspect the immutable result and request seam, and roll back by repointing to a retained predecessor without recompilation or row mutation.
...
> **Digest prompt:** You lose the network response immediately after confirming publication. Describe the safe verification sequence before retrying, and explain why blindly issuing another activation command could violate the intended compare-and-swap workflow in production.
...
Editorial governance is also unfinished product work. Who may create a high-priority selector, change a default inherited by every page, replace a renderer contract, publish, or roll back? Which changes require review? How long are failed builds and former publications retained? What author content may appear in diagnostics? The ADR requires useful run, chunk, conflict, validation, pointer, and actor observability while prohibiting unrestricted author content or page context in logs. The local HUD proves the operations, not their authorization model.
...
>
> **Open decision** — Every item in the ledger needs an accountable owner, chosen option, decision date, required evidence, rollback/revisit condition, and link to its Linear issue. “Use the prototype behavior” is not an acceptable default rationale.
...
A strong handoff lets another person reconstruct the argument without trusting the presenter. Start with scope: the CMS project in Linear is authoritative, from `AUT-514` through `AUT-532`, and every substantive change maps to an issue and acceptance criteria. `AGENTS.md` records the repository-wide contract and workflow. `docs/process-engineering-guide.md` explains the model and editorial operations. `docs/benchmarks.md` indexes correctness and scale evidence. `docs/evidence/bounded-report.json` and `docs/evidence/store-1m.json` preserve machine-readable provenance. `docs/adr/0001-tidb-materialization.md` states the accepted production direction and its blockers.
...
The automated gate supplies broad evidence after that human trace. `bun run five-phase-pass` passed on 2026-08-29 from the clean final implementation tree described in `docs/benchmarks.md`. The workspace total was 124 tests and 1,552 assertions: database 8/57, domain 62/750, service 10/122, scenarios 6/97, and CMS UI/router/server/tutorial 38/526. Fallow reported zero unused-file, export, dependency, cycle, unresolved-import, boundary/policy, or stale-suppression issues. The gate also verified the frozen install, skill-import digest, schema-v6 reset/seed, migrations, Biome, TypeScript, evidence envelopes, client/SSR/Nitro build, and cross-workspace boundaries.
...
>
> **Measured finding** — The clean gate passed 124 tests/1,552 assertions. Browser smoke rendered `/`, all three workbenches, `/publications/stores`, and `/tutorial` on live SQLite schema v6 with a clean console.

Browser evidence connects the suites to an author’s experience. Store persisted a fifth block, published two pages, and retained both the block and second immutable publication after reload. Eligible exposed seven editable placements. Structural displayed 23 visible placements, one tombstone, one `hero_alt` winner, and 22 inherited placements. The publication route read the same live pointer, document hash, and two-publication history as the workspace. Earlier smoke advanced the bounded instance page and exercised a live-without-document `503` with zero selector SQL.

The tutorial audit rendered six native player instances from five distinct reviewed capture bundles: the three scenario proofs, Wall navigation, resolution-pin inspection, and one in-place Chapter 5 replay. Every instance exposed two source formats and one description track; all 65 rendered IDs were unique. Switching the Chapter 5 selector replaced its player in place, retained keyboard-visible pressed state, and left no backward scenario-player links. MP4/WebM probes, monotonic WebVTT timing, exact transcript text, byte counts, SHA-256 integrity, posters, and manifest/schema-state coherence remain checked by the media suite. The clips remain visibly labeled reviewed previews, while approved integration stays fail-closed until final Linear/source approval.

Do not overstate this closure. The clean local gate proves the prototype contract on its recorded environment. It does not prove TiDB throughput, distributed transactions, Region behavior, cache topology, production Camo traffic, operational ownership, or authorization. The ADR’s seven spikes remain blockers: chunk/activation, selector plans, partition/global index, manifest storage, incremental equivalence, constraints/immutability, and the Camo revision seam. Local timing and allocation numbers remain **Measured finding**, never production SLO.
...
The architecture’s durable conclusion is narrow and valuable. Relational, selector-scoped authoring can remain expressive while publication compiles one deterministic, immutable, cacheable document per URL. Stable placement keys preserve editorial identity across content, order, hiding, and type replacement. Camo can remain route authority during the transition. Structural manifests are promising for sparse templates but not dogma. Full rebuild remains the oracle while production publication becomes chunked and pointer-atomic. Everything beyond those claims stays attached to evidence or an owner.

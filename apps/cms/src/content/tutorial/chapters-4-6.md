...
Publishing turns the draft relation into the serving contract. In Template workspace, the rocket icon opens **Review publication**. Its read-only preflight compiles every eligible page in the selected template, validates selectors and block schemas, detects conflicts, canonicalizes winners and provenance, and reports page impact and manifest reuse before the author can confirm. Confirmation writes immutable manifests and page documents and only then moves the serving pointer. The UI disables controls while a mutation is pending and announces every state change. A failure leaves the former pointer current.
...
Use the history icon in Template workspace to open `/publications/$templateId`. That URL-backed view is the selected template's **Release history**, not a global publication dashboard and not a second publishing workflow. It is defined in `apps/cms/src/routes/publications.$templateId.tsx` and renders `apps/cms/src/components/publication-inspection.tsx` from live SQLite rows. The compact template selector changes template while staying in release history; the authoring icon returns to the same canonical page context.

The summary distinguishes three different facts: the immutable publication currently named by `current_publications`, its exact retained predecessor when one exists, and the total immutable records owned by the template. The table orders real sequences newest first and exposes author, published time, page and manifest counts, input hash, and predecessor ID. Filters can isolate the active release, rollback target, or older history. The page is deliberately read-only: **Open authoring to review changes** returns to Template workspace, where the rocket control runs the guarded preflight, so a fixture card or generic mutation cannot bypass the input-hash and expected-pointer checks.
...
>
> **Current implementation** — The local SQLite service performs the bounded write and pointer activation transactionally. Release history performs a bounded, template-scoped read from SQLite; all publish and rollback mutations remain in the authoring preflight controller.
>
...
Before rollback, remember the request boundary proven by the service tests: a `live` page plus an active publication produces `200`; missing, `not_live`, `archived`, and live-but-unpublished pages produce `404` with distinct reasons. The public route calls the materialized `CmsService.serve` path and never runs selector SQL. Release history reports the template pointer but does not emulate or mutate a public request.
...
To roll back, return to Template workspace and open **Review publication**. The panel names `workspace.rollbackPublicationId`, the actual retained predecessor. Confirm that it is present and that the expected current publication has not changed. **Confirm rollback** repoints serving to that immutable namespace; no publication page or manifest row is rewritten. After the mutation, reopen Release history and verify its active and rollback badges, then verify the exact document hash on the public route. The former current publication remains immutable history.
...
The one-minute media should show a hash-centered workflow. Capture the active ID and hash, run and confirm publication preflight in Template workspace, verify the new active release in Release history, then return to authoring and confirm rollback. End with the original public hash restored. Narrate the difference between a saved draft, a read-only preflight, an immutable release row, and the active serving pointer. Provide captions for status changes, and do not use a green/red transition as the only signal of success or failure.
